...
Regression (found 2026-06-06): the `is_environment` gate does not protect garden -- the scene fails the hollow-shell heuristic (`inner_density_ratio >= 0.05`, the table fills the AABB center), so the cull runs on an environment scan. ~670 interior hulls culled (under-canopy and under-table geometry is invisible from all 32 exterior viewpoints but collidable from inside), coverage 98.3% -> 51% against the identical config of the pre-phase-3 retry bundle. The docstring's own caveat ("not valid for environments") is doing load-bearing work the heuristic can't back up. Fix options: make the cull coverage-aware (only cull hulls whose input points stay covered by the remaining union), or stop gating on a heuristic that misses indoor-ish scenes. Open -- this is the next work item, ahead of phase 4 and the `as_completed` ordering hardening.
