...
Determinism is not free, and one dependency has to be held to it deliberately. CoACD's search is OpenMP-parallel, and its thread scheduling decides which decomposition it settles on: run unpinned, the same mesh returns a different hull count and different bytes every time (measured: 47/48/50 hulls over four runs of one 3.6k-face mesh, four distinct hashes). Chitin therefore runs the CoACD worker with `OMP_NUM_THREADS=1` by default, which costs 2-4x wall time on concave assets and buys byte-identical output. `--fast` (`Config(coacd_deterministic=False)`) trades that back for speed; the choice is recorded in the build plan and in `resolved-config.json`, and the `robotics` profile rejects a build that took it. This is the same fix, for the same reason, as the single-threaded Poisson note below.
...
5. **decompose** -- CoACD convex decomposition, LOD tier generation, walkable hull extraction, cross-cell AABB IOU deduplication. CoACD runs in a subprocess, single-threaded for reproducibility, under a time budget (`--coacd-timeout`, default 300s) that hard-kills a native stall and substitutes a bounding box
...
7. **segment** -- bone segmentation for rigged assets: assign each vertex to its dominant bone. Each bone decomposes under its own build plan so per-bone pipeline steps stay off the asset's, with the acceptance-relevant signals merged back; the same applies to octree cells, which decompose in a separate process and carry their signals home in the result. Counters (CoACD timeouts, fallback hulls) merge by sum, flags (was the decomposition reproducible) by AND, so one unpinned piece is not hidden by the pieces around it
...
**Environment scans are auto-detected with explicit opt-out.** Poisson creates watertight meshes, which fills concave environments with solid geometry. The proximity filter and thin-shell extrusion fix this. Chitin detects them on two independent signals: a hollow-shell point distribution (fewer than 5% of points in the inner 50% of the AABB), or a shell signature -- a thin floor plane plus two or more thin wall planes covering their AABB faces. The second exists because clutter defeats the first: a room with a central pillar or mid-floor shelving has high inner density but keeps its walls. Either signal enables both fixes. Use `auto_environment=False` or `--no-auto-environment` to disable, `force_environment=True` or `--environment` to force it; ambiguous inputs (density in the 0.05-0.20 band with no shell signature) run as solid objects and say so in `chitin check`.
