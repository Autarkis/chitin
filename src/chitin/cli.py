...
def _add_extract_parser(sub: argparse._SubParsersAction) -> None:
    ...
    p.add_argument(
        ...
        type=int,
        default=2048,
        help="Max convex hulls per decomposition unit (per source-mesh component / "
        "octree cell / bone), not a global cap (default: 2048)",
    )
    p.add_argument(
        "--lod-concavities",
        type=str,
...
def _cmd_extract(args: argparse.Namespace) -> None:
    ...
    # both the coverage probe and capsule sweep required by the walkable gate.
